//coded by Jason Thong for CoE3DQ5 2013
//decoding of milestone 3 - read a .mic7 file, do lossless decoding, write a .sram_d2 file
...
int main(int argc, char **argv) {
	int i, j, k, m, n, r, color, width, height, width_temp, sram_offset_address, decoded_data[64], q[15];
	...
	if (argc<2) {
		printf("enter the input file name including the .mic7 extension: ");
		gets(input_filename);
	}
...
*/
			for (k=0; k<64; k++) {
				m = read_bits(file_ptr, 2);	//read first 2 bits of header
				if (m==0) {	//9 bit coefficient
					n = read_bits(file_ptr, 9);
					decoded_data[zigzag_order[k]] = (n >= 256) ? (n-512) : n;
				}
				else if (m==1) {	//4 bit coefficient
					n = read_bits(file_ptr, 4);
					decoded_data[zigzag_order[k]] = (n >= 8) ? (n-16) : n;
				}
				else {	//m is 2 or 3, after shifting will be 4 or 6, getting lsb may shift it to 5 or 7
					m = (m<<1) | read_bits(file_ptr, 1);	//get the third bit of header
					if (m==4) {	//run of -1
						n = read_bits(file_ptr, 2);
						if (n==0) n = 4;
						for (r=0; r<n; r++) {
							decoded_data[zigzag_order[k]] = -1;
							if (r+1<n) k++;
						}
					}
					else if (m==5) {	//run of +1
						n = read_bits(file_ptr, 2);
						if (n==0) n = 4;
						for (r=0; r<n; r++) {
							decoded_data[zigzag_order[k]] = 1;
					...
					else if (m==6) {	//run of 0
						n = read_bits(file_ptr, 3);
						if (n==0) n = 8;
						for (r=0; r<n; r++) {
							decoded_data[zigzag_order[k]] = 0;
							if (r+1<n) k++;
						}
					}
					else {	//run of 0 to end
